...
$module_params = func_query_first("SELECT * FROM $sql_tbl[ccprocessors] WHERE processor='cc_targetpay_paysafe.php'");

if (!isset($REQUEST_METHOD)) {
    $REQUEST_METHOD = $_SERVER["REQUEST_METHOD"];
}
    
if (!func_is_active_payment('cc_targetpay_paysafe.php')) {
    exit;    
}
...
if ($REQUEST_METHOD == "GET" && isset($_GET['trxid'])  && ($_GET['return'] == 'success' || $_GET['return'] == 'cancel') || $_GET['return'] == 'callback') {
    ...
    $targetPay = new TargetPayCore("WAL", $module_params['param01'], "382a92214fcbe76a32e22a30e1e9dd9f", "nl", $testmode);
    ...
    db_query("UPDATE $sql_tbl[orders] SET payment_method = 'Targetpay - Paysafe Card' WHERE orderid='". clean($tpOrder->order_id) ."' LIMIT 1");
    ...
    if ($_GET['return'] == 'success' || $_GET['return'] == 'cancel') {
        if ($paid) {
            ...
            // 20170110 - Tyson - Changed the status from 'Q' to 'P' to match with ideal mrcash
    ...
    $targetPay->setReturnUrl($current_location."/payment/cc_targetpay_paysafe.php?return=success");
    $targetPay->setCancelUrl($current_location."/payment/cc_targetpay_paysafe.php?return=cancel");
    $targetPay->setReportUrl($current_location."/payment/cc_targetpay_paysafe.php?return=callback");
    ...
    if ($url) {
        ...
        $sql = "INSERT INTO `targetpay_sales` SET `order_id` = '".$secure_oid[0]."', `method` = 'WAL', `amount` = '".$amount."', `targetpay_txid` = '".$targetPay->getTransactionId()."'";
        db_query($sql);
