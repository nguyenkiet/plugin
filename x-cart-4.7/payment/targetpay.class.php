...
class TargetPayCore
{
    ...
    public function startPayment($noBank = false) 
    {
        ...
        $url =    "https://www.targetpay.com/api/idealplugins?".
          "paymethod=".urlencode($this->payMethod)."&".
                ...
                (($this->payMethod=="DEB") ? "type=1&country=49&lang=".urlencode($this->getLanguage(array("NL","EN","DE"), "DE"))."&" : "").
     ...
     *    @param string $transactionId Transaction ID to check Returns true if payment successfull (or testmode) and false if not After payment: - Read the errors with getErrorMessage() - Get user information using getConsumerInfo() Returns true if payment successfull (or testmode) and false if not After payment: - Read the errors with getErrorMessage() - Get user information using getConsumerInfo()
     *    
